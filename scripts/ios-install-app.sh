...
if [[ -n "${PRBAR_IOS_POSTHOG_HOST:-}" ]]; then
  XCODEBUILD_EXTRA_ARGS+=("PRBAR_IOS_POSTHOG_HOST=$PRBAR_IOS_POSTHOG_HOST")
fi

if [[ -n "${PRBAR_IOS_POSTHOG_PROJECT_ID:-}" ]]; then
  XCODEBUILD_EXTRA_ARGS+=("PRBAR_IOS_POSTHOG_PROJECT_ID=$PRBAR_IOS_POSTHOG_PROJECT_ID")
fi

if [[ -n "${PRBAR_IOS_POSTHOG_PERSONAL_API_KEY:-}" ]]; then
  XCODEBUILD_EXTRA_ARGS+=("PRBAR_IOS_POSTHOG_PERSONAL_API_KEY=$PRBAR_IOS_POSTHOG_PERSONAL_API_KEY")
fi

if [[ -n "${PRBAR_IOS_POSTHOG_DASHBOARD_ID:-}" ]]; then
  XCODEBUILD_EXTRA_ARGS+=("PRBAR_IOS_POSTHOG_DASHBOARD_ID=$PRBAR_IOS_POSTHOG_DASHBOARD_ID")
fi
