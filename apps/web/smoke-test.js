...
const routeExpectations = [
  ...
  ["#/project", "Proof attaches to the things Maya shipped."],
...
async function runChecks(page) {
  ...
  await assertBodyIncludes(page, "/home signed-in dashboard action", "Open profile dashboard");
  ...
  await assertBodyIncludes(page, "/profile featured receipt", "Facts are locked from GitHub");
  ...
  await assertBodyIncludes(page, "/profile persisted source count", "3 selected sources power the card, receipt, app proof, and timeline.");

  await page.locator('[data-preview-action="enter"]').click();
  ...
  await assertBodyExcludes(page, "/profile public preview owner controls", "Owner controls");
  ...
  for (const [route, expectedText, selector] of [
    ...
    await page.goto(`${baseUrl}/${route}`, { waitUntil: "networkidle" });
    await assertBodyIncludes(page, route, expectedText);
    await assertBodyIncludes(page, route, "deep link");
